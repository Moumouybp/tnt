...
def get_fc_edge_index(node_indices):
    ...
    return a tensor(2, edges), indicing edge_index      实现节点全连接
    """
...
def get_traj_edge_index(node_indices):
    ...
    node_indices: np.array([indices]), the indices of nodes connecting with each other;
    return a tensor(2, edges), indicing edge_index   返回子图中连接关系
    """
...
class ArgoverseInMem(InMemoryDataset):
    ...
    def process(self):
        """ transform the raw data and store in GraphData     处理特征，转为GraphData"""
        # loading the raw data
        traj_lens = []
        valid_lens = []
        candidate_lens = []
        for raw_path in tqdm(self.raw_paths, desc="Loading Raw Data..."):# 逐个读取.pkl文件
            raw_data = pd.read_pickle(raw_path)

            # statistics
            traj_num = raw_data['feats'].values[0].shape[0] # agent的观测轨迹
            traj_lens.append(traj_num)

            lane_num = raw_data['graph'].values[0]['lane_idcs'].max() + 1 # 车道个数
            valid_lens.append(traj_num + lane_num)

            candidate_num = raw_data['tar_candts'].values[0].shape[0] # 预测车道的个数
            candidate_lens.append(candidate_num)
        num_valid_len_max = np.max(valid_lens)  # 所有场景中，轨迹和车道数最多
        ...
        for ind, raw_path in enumerate(tqdm(self.raw_paths, desc="Transforming the data to GraphData...")):
            ...
            y = self._get_y(raw_data) # 计算预测轨迹的相对偏移量
    ...
    def get(self, idx): # 在DataLoader访问时，会调用这个函数，继续完善graph
        data = super(ArgoverseInMem, self).get(idx).clone()

        feature_len = data.x.shape[1]
        index_to_pad = data.time_step_len[0].item()  # 最多有多少子图，需要把每个图填充
        ...
        data.candidate_mask = torch.cat([torch.ones((len(data.candidate), 1)), # 使用掩码说明填充的候选车道
    ...
    def _get_x(data_seq):
        ...
        for cluster_idc in np.unique(cluster):
            [indices] = np.where(cluster == cluster_idc) # 找到cluster中所有id==idc的行索引
            identifier = np.vstack([identifier, np.min(feats[indices, :2], axis=0)])
            # 取出每个聚类的最小坐标（xys或者ctrs）
            if len(indices) <= 1:
                continue                # skip if only 1 node
            if cluster_idc < traj_cnt:
                edge_index = np.hstack([edge_index, get_fc_edge_index(indices)]) # 这些聚类就是同一个子图，建立全连接边
            else:
    ...
    @staticmethod
    def _get_y(data_seq):
        traj_obs = data_seq['feats'].values[0][0] # 轨迹点
        traj_fut = data_seq['gt_preds'].values[0][0] # 真值点
        offset_fut = np.vstack([traj_fut[0, :] - traj_obs[-1, :2], # 第一个gt点相对于最后一个观测点的偏移
                                traj_fut[1:, :] - traj_fut[:-1, :]]) # 后续gt点之间的相对偏移
        return offset_fut.reshape(-1).astype(np.float32) # 展平数据返回
